...
class FactBox extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.country === "Iceland") {
            let index = Math.floor(Math.random() * icelandFacts.length);
            this.setState({
                fact: icelandFacts[index]
            });
            tempRemovedIcelandFacts.push(icelandFacts.splice(index, 1)[0]);
            if (icelandFacts.length === 0) {
                let temp = icelandFacts.concat(tempRemovedIcelandFacts);
                icelandFacts = temp;
                tempRemovedIcelandFacts = [];
            }
        } else if (nextProps.country === "Finland") {
            let index = Math.floor(Math.random() * finlandFacts.length);
            this.setState({
                fact: finlandFacts[index]
            });
            tempRemovedFinlandFacts.push(finlandFacts.splice(index, 1)[0]);
            if (finlandFacts.length === 0) {
                let temp = finlandFacts.concat(tempRemovedFinlandFacts);
                finlandFacts = temp;
                tempRemovedFinlandFacts = [];
            }
        } else if (nextProps.country === "Sweden") {
            let index = Math.floor(Math.random() * swedenFacts.length);
            this.setState({
                fact: swedenFacts[index]
            });
            tempRemovedSwedenFacts.push(swedenFacts.splice(index, 1)[0]);
            if (swedenFacts.length === 0) {
                let temp = swedenFacts.concat(tempRemovedSwedenFacts);
                swedenFacts = temp;
                tempRemovedSwedenFacts = [];
            }
        } else if (nextProps.country === "Norway") {
            let index = Math.floor(Math.random() * norwayFacts.length);
            this.setState({
                fact: norwayFacts[index]
            });
            tempRemovedNorwayFacts.push(norwayFacts.splice(index, 1)[0]);
            if (norwayFacts.length === 0) {
                let temp = norwayFacts.concat(tempRemovedNorwayFacts);
                norwayFacts = temp;
                tempRemovedNorwayFacts = [];
            }
        } else if (nextProps.country === "Denmark") {
            let index = Math.floor(Math.random() * denmarkFacts.length);
            this.setState({
                fact: denmarkFacts[index]
            });
            tempRemovedDenmarkFacts.push(denmarkFacts.splice(index, 1)[0]);
            if (denmarkFacts.length === 0) {
                let temp = denmarkFacts.concat(tempRemovedDenmarkFacts);
                denmarkFacts = temp;
                tempRemovedDenmarkFacts = [];
            }
        }
    }
